...
export async function PATCH(request: NextRequest) {
  ...
  const booleanFields = ["accessWheelchair", "accessToilets", "accessParking", "accessElevator", "accessLevelEntry"] as const
  for (const bf of booleanFields) {
    if (bf in data && typeof data[bf] !== "boolean") return NextResponse.json({ error: `Invalid ${bf}` }, { status: 400 })
  }
